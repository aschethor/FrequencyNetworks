...
class Model(nn.Module):
	...
	def __init__(self,n_input_images,image_shape=[100,100],hidden_size=20,share_U=True,share_V=True,share_W=True):
		...
		self.u1 = nn.Conv2d( 3, hidden_size, kernel_size=5,padding = 2,bias=False)
		self.v1 = nn.Conv2d( 3, hidden_size, kernel_size=5,padding = 2,bias=False)
		self.w1 = nn.Conv2d( hidden_size, hidden_size, kernel_size=5,padding = 2,bias=False)
		
		self.u1_prime = nn.Conv2d( 3, hidden_size, kernel_size=5,padding = 2,bias=False)
		self.v1_prime_t = nn.ConvTranspose2d( hidden_size, 3, kernel_size=5, padding=2,bias=False)
		self.w1_prime_t = nn.ConvTranspose2d( hidden_size, hidden_size, kernel_size=5, padding=2,bias=False)
