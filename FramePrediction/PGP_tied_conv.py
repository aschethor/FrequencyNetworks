...
class Model(nn.Module):
	...
	def __init__(self,n_input_images,image_shape=[100,100],hidden_size=20,share_U=True,share_W=True):
		...
		self.u1_t = nn.Conv2d( 3, hidden_size, kernel_size=5, padding=2, bias=False)
		self.w1 = nn.Conv2d( hidden_size, hidden_size, kernel_size=5, padding=2, bias=True)
		
		self.u1_prime = nn.ConvTranspose2d( hidden_size, 3, kernel_size=5, padding=2, bias=True)
		...
		self.P1 = P.cuda()
		self.P2 = P.cuda()#probably another multiplication by R needed here... (torch.mm(R,P))
	...
	def forward(self,x):
		...
		x = torch.sigmoid(self.w1_prime_t(x, output_size = self.image_shape))
